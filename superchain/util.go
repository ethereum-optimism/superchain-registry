...
func (b *Address) UnmarshalText(text []byte) error {
	if err := decodeHex(b[:], text); err != nil {
		return fmt.Errorf("failed to decode address: %w", err)
	}
	if expected := checksumAddress(*b); expected != string(text) {
		return fmt.Errorf("invalid address checksum, expected %q but got %q", expected, string(text))
	}
	return nil
}
