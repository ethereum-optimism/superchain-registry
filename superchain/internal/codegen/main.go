...
#  DO NOT REMOVE - USED BY BINDINGS          #
##############################################

`)
		// Write configs.toml file to the top-level directory.
		buf = *bytes.NewBuffer(append(header, buf.Bytes()...))
		err = os.WriteFile(filepath.Join(repositoryRoot, "superchain/configs/configs.toml"), buf.Bytes(), 0o644)
		if err != nil {
			panic(err)
		}
