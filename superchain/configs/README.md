...
| --- | --- | --- |
| **immutable** | Fixed when the chain is created. Either it identifies the chain, or it is a consensus parameter whose **source of truth is L1** (the registry only mirrors it) and that changes *only* via a network-synchronized mechanism such as an L1 `SystemConfig` event. No job currently syncs these back into the source configs and there is no process for changing them intentionally, so an edit here is almost always accidental. (Contrast append-only hardfork times, where the registry itself *is* the source of truth.) | `chain_id`, `batch_inbox_addr`, `block_time`, `seq_window_size`, `max_sequencer_drift`, `gas_paying_token`, `data_availability_type`, `[optimism]` (EIP-1559 params), `[interop]` dependencies, the entire `[genesis]` block (including `[genesis.system_config]`) |
