...
contract CheckSecurityConfigs is Script {
        ...
        if (block.chainid == 1) {
            ...
            revert(
                string.concat(
                    ...
                    ". Please call runOnDir(string) directly."
                )
            );
        }
        string memory jsonDir = string.concat(
            "superchain/extra/addresses/",
            network
        );
        runOnDir(jsonDir);
    }
    ...
    function runOnDir(string memory jsonDir) public {
        ...
        for (uint256 i = 0; i < addressesJsonEntries.length; i++) {
            require(
                bytes(addressesJsonEntries[i].errorMessage).length == 0,
                addressesJsonEntries[i].errorMessage
            );
    ...
    function chainUpgradedToFPAC(
        string memory addressesJsonPath
        ...
        return
            keccak256(
                abi.encodePacked(
                    sliceString(
                        addressesJsonPath,
                        bytes(addressesJsonPath).length - 15,
                        bytes(addressesJsonPath).length
                    )
                )
    ...
    function checkL1CrossDomainMessengerProxy(
        ProtocolAddresses memory addresses
    ) internal {
        console2.log(
            "Checking L1CrossDomainMessengerProxy %s",
    ...
    function checkL1ERC721BridgeProxy(
        ProtocolAddresses memory addresses
    ) internal {
        console2.log(
            "Checking L1ERC721BridgeProxy %s",
            addresses.L1ERC721BridgeProxy
        );
    ...
    function checkL1StandardBridgeProxy(
        ProtocolAddresses memory addresses
    ) internal {
        console2.log(
            "Checking L1StandardBridgeProxy %s",
    ...
    function checkL2OutputOracleProxy(
        ProtocolAddresses memory addresses,
        bool upgradedToFPAC
    ) internal {
        if (upgradedToFPAC) {
        ...
        console2.log(
            "Checking L2OutputOracleProxy %s",
    ...
    function checkOptimismMintableERC20FactoryProxy(
        ProtocolAddresses memory addresses
    ) internal {
        console2.log(
            "Checking OptimismMintableERC20FactoryProxy %s",
    ...
    function checkOptimismPortalProxy(
        ProtocolAddresses memory addresses,
        bool upgradedToFPAC
    ) internal {
        console2.log(
            "Checking OptimismPortalProxy %s",
            addresses.OptimismPortalProxy
        );
    ...
    function checkSystemConfigProxy(
        ProtocolAddresses memory addresses
    ) internal {
        console2.log(
            "Checking SystemConfigProxy %s",
            addresses.SystemConfigProxy
        );
    ...
    function getMappingValue(
        ...
        bytes32 slotValue = vm.load(
            targetContract,
            keccak256(abi.encode(key, mapSlot))
        );
    ...
    function checkAddressIsExpected(
    ...
    function getAddressFromCall(
        address contractAddr,
        string memory signature
    ) internal returns (address) {
        vm.prank(address(0));
        (bool success, bytes memory addrBytes) = contractAddr.staticcall(
    ...
    function getAddresses(
        string memory addressesJsonPath
    ) internal view returns (ProtocolAddresses memory) {
        string memory addressesJson = vm.readFile(addressesJsonPath);
        return
            ProtocolAddresses({
                AddressManager: vm.parseJsonAddress(
                    addressesJson,
                    ".AddressManager"
                ),
                L1CrossDomainMessengerProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".L1CrossDomainMessengerProxy"
                ),
                L1ERC721BridgeProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".L1ERC721BridgeProxy"
                ),
                L1StandardBridgeProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".L1StandardBridgeProxy"
                ),
                L2OutputOracleProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".L2OutputOracleProxy"
                ),
                OptimismMintableERC20FactoryProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".OptimismMintableERC20FactoryProxy"
                ),
                OptimismPortalProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".OptimismPortalProxy"
                ),
                ProxyAdmin: vm.parseJsonAddress(addressesJson, ".ProxyAdmin"),
                SystemConfigProxy: vm.parseJsonAddress(
                    addressesJson,
                    ".SystemConfigProxy"
                ),
                Challenger: vm.parseJsonAddress(addressesJson, ".Challenger"),
                Guardian: vm.parseJsonAddress(addressesJson, ".Guardian"),
                ProxyAdminOwner: vm.parseJsonAddress(
                    addressesJson,
                    ".ProxyAdminOwner"
                ),
                SystemConfigOwner: vm.parseJsonAddress(
                    addressesJson,
                    ".SystemConfigOwner"
                )
            });
