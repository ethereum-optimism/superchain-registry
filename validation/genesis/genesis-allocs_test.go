	...
	if entry, ok := weth9ExpectedData[key].(map[string]interface{}); ok {
		if code, ok := entry["code"].(string); ok {
			...
			if len(code) > 100 {
				...
				fmt.Println("The regenerated bytecode length is less than 100 characters; no trimming performed.")
			}
		} else {
			log.Fatalf("Field 'code' not found or is not a string in regenerated bytecode: %s", key)
		}
	} else {
