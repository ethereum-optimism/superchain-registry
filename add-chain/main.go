...
func entrypoint(ctx *cli.Context) error {
	...
	// Ensure rollup config generated by op-node is equivalent to
	// pre-existing one used to create superchain config files
	opNodeRollupConfig, err := rollup.LoadOPStackRollupConfig(rollupConfig.ChainID)
	if err != nil {
		return fmt.Errorf("failed when using op-node to generate rollup config: %w", err)
	}
	...
	if !reflect.DeepEqual(opNodeRollupConfig, localRollupConfig) {
		return fmt.Errorf("local rollup.json does not match config generated by op-node")
	}
