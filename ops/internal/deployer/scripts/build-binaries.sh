...
build_from_source() {
  ...
  # Trust the source directory
  echo "Trusting source directory for mise..."
  mise trust "$source_dir"

  # Explicitly install Go using mise
  ...
  if ! mise exec -- go build -o bin/op-deployer ./cmd/op-deployer/main.go; then
